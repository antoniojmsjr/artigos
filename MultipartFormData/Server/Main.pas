...
//    Log(lJSONCadastro.ToString);
  ...
  begin
    if (pReq.RawWebRequest.Files.Items[I].FieldName = 'perfil') then
    begin
      lFilePerfil := TMemoryStream.Create;
      try
        lFilePerfil.LoadFromStream(pReq.RawWebRequest.Files.Items[I].Stream);
        lFilePerfil.Position := 0;
        lFilePerfil.SaveToFile(ExtractFilePath(ParamStr(0)) + pReq.RawWebRequest.Files.Items[I].FileName);
        lblPerfil.Caption := Format('%s%s', [ExtractFilePath(ParamStr(0)), pReq.RawWebRequest.Files.Items[I].FileName]);
      finally
        lFilePerfil.Free;
      end;
    end;
